...
6. 검색/그래프/정리는 `markdown-manager`, 작성/보고서/덱/렌더링은 `markdown-writer` 사용을 요청합니다.
...
기본 설치는 `markdown-manager`와 `markdown-writer` 두 개를 설치합니다. 사용자는 많은 하위 스킬을 고를 필요 없이, 찾기/그래프/정리는 `/markdown-manager`, 작성/보고서/덱/렌더링은 `/markdown-writer`로 시작하면 됩니다.

세부 스킬을 직접 설치하고 싶을 때만 `Advanced: choose source and target`를 사용하면 됩니다.

### 에이전트 채팅에서는 이렇게 사용합니다

설치가 끝나면 AI 에이전트 채팅에서 작업 성격에 맞는 스킬을 첫 지시로 넣으면 됩니다.

- `/markdown-manager`: 워크스페이스 검색, Source Graph, 링크, 업데이트 영향, canonical 문서, `.mpsignore`
- `/markdown-writer`: 작성, 재구성, 보고서, 블로그용 Markdown, 발표형 Markdown, 덱 준비, HTML 렌더 점검

slash command가 없는 일반 채팅에서는 `markdown-manager를 사용해줘` 또는 `markdown-writer를 사용해줘`라고 쓰면 됩니다.

어디서 시작해야 할지 모르겠다면 사용법 자체도 에이전트에게 먼저 물어볼 수 있습니다.

```text
markdown-manager를 사용해줘.

Agent Docs를 처음 쓰는 사람 기준으로 Source Graph와 내장 스킬 사용법을 알려줘.
언제 markdown-manager를 쓰고, 언제 markdown-writer를 써야 하는지 예시 프롬프트와 함께 설명해줘.
```
...
```text
markdown-writer를 사용해서 @brief.md를 Agent Docs 보고서로 재구성해줘. 근거는 유지하고 구조를 개선한 뒤 export 점검 목록까지 넣어줘.
```
...
| --- | --- | --- |
| `markdown-manager` | Markdown 검색, 그래프 정리, 링크 점검, 업데이트 계획, canonical 문서 판단, context packaging | `markdown-manager를 사용해서 wiki/concepts/agentic-ai.md를 수정하기 전에 관련 문서와 깨진 링크를 찾아줘.` |
| `markdown-writer` | 보고서, 브리프, 튜토리얼, 발표형 Markdown, 덱 준비, export/render 점검 | `markdown-writer를 사용해서 @brief.md를 근거 중심 Agent Docs 보고서로 재구성하고 렌더 점검까지 해줘.` |
...
| `markdown-writer` | 작성, 발표형 Markdown, 덱 준비 구조, export/render 점검 | `markdown-writer를 사용해서 이 리서치 노트를 임원 보고서로 재구성하고 HTML 렌더 결과까지 점검해줘.` |
